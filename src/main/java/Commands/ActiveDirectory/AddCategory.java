...
public class AddCategory extends BaseCommand {
    ...
    public AddCategory() {
        super(
                ...
                "addcategory",
                new String[] {  },
                "addcategory [unit_name] [category_name]",
            ...
            if (Arrays.asList(availableCategories).contains(suppliedCategory)) {
                ...
                CommandHandler.getCommands().forEach(command -> {
                    ...
                    if (CommandHandler.getCommand(command).getCategory().equalsIgnoreCase(args[1])) {
                        ...
                        ActiveDirectoryManagement.createCommand(event.getGuild().getId(), args[0], command);

                    }

                });

                event.getChannel().sendMessage("Successfully added commands from the specified category.").queue();
            } else {
